...
def candidate_reports():
    """
    Select synchronized BUY/SELL exports as a PAIR, never independently.
    Authority order:
      1) same closed-trade count
      2) exact entry timestamp vector match
      3) highest synchronized trade count
      4) newest combined modification time
    """
    found=[]
    for p in DL.rglob("*.csv"):
        try:
            head=pd.read_csv(p,nrows=20)
            if not REQ.issubset(set(head.columns)):
                continue
            side=classify_side(head)
            if not side:
                continue

            slim=pd.read_csv(p,usecols=["Trade number","Type","Date and time"])
            ex=slim[slim["Type"].astype(str).str.startswith("Exit")]
            en=slim[slim["Type"].astype(str).str.startswith("Entry")]
            n_exit=len(ex); n_entry=len(en)
            if n_exit<100 or n_entry!=n_exit:
                continue

            entry_times=tuple(en.sort_values("Trade number")["Date and time"].astype(str).tolist())
            ts_hash=hashlib.sha256("\n".join(entry_times).encode()).hexdigest().upper()
            found.append({
                "path":p,"side":side,"trades":n_exit,"mtime":p.stat().st_mtime,
                "entry_ts_hash":ts_hash,"entry_times":entry_times
            })
        except Exception:
            pass

    buys=[x for x in found if x["side"]=="BUY"]
    sells=[x for x in found if x["side"]=="SELL"]
    if not buys:
        raise RuntimeError("No valid full TradingView BUY trade-list CSV found in Downloads.")
    if not sells:
        raise RuntimeError("No valid full TradingView SELL trade-list CSV found in Downloads.")

    pairs=[]
    for b in buys:
        for s in sells:
            if b["trades"]!=s["trades"]:
                continue
            exact=(b["entry_ts_hash"]==s["entry_ts_hash"] and b["entry_times"]==s["entry_times"])
            if not exact:
                continue
            pairs.append({
                "BUY":b,"SELL":s,
                "trades":b["trades"],
                "combined_mtime":b["mtime"]+s["mtime"]
            })

    if not pairs:
        buy_counts=sorted({x["trades"] for x in buys},reverse=True)[:10]
        sell_counts=sorted({x["trades"] for x in sells},reverse=True)[:10]
        raise RuntimeError(
            "No synchronized BUY/SELL export pair found. "
            f"BUY counts={buy_counts}; SELL counts={sell_counts}. "
            "Need full-history exports with identical entry timestamps."
        )

    pairs.sort(key=lambda z:(z["trades"],z["combined_mtime"]),reverse=True)
    win=pairs[0]

    audit_rows=[]
    for z in sorted(pairs,key=lambda q:(q["trades"],q["combined_mtime"]),reverse=True):
        audit_rows.append({
            "selected": z is win,
            "trades": z["trades"],
            "entry_timestamp_hash": z["BUY"]["entry_ts_hash"],
            "buy_path": str(z["BUY"]["path"]),
            "sell_path": str(z["SELL"]["path"]),
            "buy_mtime": z["BUY"]["mtime"],
            "sell_mtime": z["SELL"]["mtime"],
        })

    return {"BUY":win["BUY"]["path"],"SELL":win["SELL"]["path"]}, audit_rows
...
def main():
    ...
    reports,selection_audit=candidate_reports()
    ...
    selection_csv=RUNS/f"X10_3C_EXPORT_PAIR_SELECTION_{run}.csv"
    pd.DataFrame(selection_audit).to_csv(selection_csv,index=False)
    ...
    lines=[
        "="*128,
        "FLIP FLOP X10.3C-R1.1 TARGET CEILING AUTOPSY",
        "="*128,
        "STATUS: PASS",
        "",
        "SOURCE AUTHORITY",
        "Selection rule: synchronized pair only = equal trade count + exact entry timestamp vector.",
        f"BUY:  {reports['BUY']}",
        f"SELL: {reports['SELL']}",
    ...
    manifest={
        "run":run,"pair_sync_sha256":sync,"summary":str(txt),"selection_audit":str(selection_csv),
    ...
    print("="*92)
    print("FLIP FLOP X10.3C-R1.1 TARGET CEILING AUTOPSY")
    print("STATUS: PASS")

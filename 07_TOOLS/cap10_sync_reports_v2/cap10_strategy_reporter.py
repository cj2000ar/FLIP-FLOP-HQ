...
REQ = ["Trade number","Type","Date and time","Signal","Size (qty)","Net PnL USD"]
...
def trades(rows):
    ...
    for n in sorted(by):
        ...
        out.append({
            ...
            "qty":qty,"pnl":pnl,"unit":pnl/qty,"weak":weak,"sync":sync
        })
    return out
...
def scenario(ts,q,kind):
    if kind=="UNIFORM_QQ":
        vals=[t["unit"]*q for t in ts]
    else:
        vals=[t["unit"]*(1 if t["weak"] else q) for t in ts]
    return vals, perf(vals)
...
def main():
    ...
    for kind in ["UNIFORM_QQ","STRONG_Q_WEAK1"]:
        for q in range(1,11):
            ...
            pairvals=[]
            for t in shared:
                b=bmap[t]["unit"]*(q if kind=="UNIFORM_QQ" or not bmap[t]["weak"] else 1)
                s=smap[t]["unit"]*(q if kind=="UNIFORM_QQ" or not smap[t]["weak"] else 1)
                pairvals.append(b+s)
            pp=perf(pairvals)
            rows.append({
                "Sizing":kind,"Q":q,
    ...
    csv_out=OUT/f"CAP10_Q1_Q10_STRATEGY_REPORT_{stamp}.csv"
    ...
    html_out=OUT/f"CAP10_Q1_Q10_STRATEGY_REPORT_{stamp}.html"

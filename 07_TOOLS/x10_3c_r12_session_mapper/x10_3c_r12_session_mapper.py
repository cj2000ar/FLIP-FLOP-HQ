...
def main():
    ...
    target_file=latest_file("X10_3C_TARGET_WINNERS_*.csv",X3C/"runs")
    audit_file=latest_file("X10_3B_FILE_SCHEMA_AUDIT_*.csv",X3B)
    ...
    lines=[
        "="*128,
        "FLIP FLOP X10.3C-R1.2A SESSION / HIGH-DETAIL OVERLAP AUDIT",
        "="*128,
        "STATUS: PASS",
        "",
        "WHY R1.2 EXISTS",
    ...
    latest_report=BASE/"LATEST_X10_3C_R12_SESSION_MAPPER_SUMMARY.txt"
    latest_report.write_text(txt.read_text(encoding="utf-8"),encoding="utf-8")
    ...
    print("="*92)
    print("FLIP FLOP X10.3C-R1.2A SESSION MAPPER")
    print("STATUS: PASS")

...
class MigrationFramework:
    ...
    def get_checksum(self, migration_number: int) -> str:
        ...
        sql_file = self.migrations_dir / f"{migration_number:04d}_*.sql"
        files = list(self.migrations_dir.glob(f"{migration_number:04d}_*.sql"))
    ...
    def apply_all_pending(self, target_migration: Optional[int] = None) -> List[int]:
        ...
        for migration_num in to_apply:
            sql_file = list(self.migrations_dir.glob(f"{migration_num:04d}_*.sql"))[0]
